...
router.put('/', upload.none(), function (request, response) {
  ...
  if (createdAccount) {
    response.json({
      success: false,
      error: 'Счёт с таким именем уже существует',
    });
    return;
  }
...
router.delete('/', upload.none(), function (request, response) {
  ...
  if (removingAccount) {
    // если удаляемый аккаунт существует
...
router.get('/:id?', upload.none(), function (request, response) {
  ...
  if (accountId) {
    ...
    if (!currentAccount) {
      response.json({
        success: false,
        error: `Счёт c идентификатором ${accountId} не найден`,
      });
      return;
    }
    let currentAccountTransactions = db
      .get('transactions')
      .filter({ account_id: currentAccount.id })
      .value();
    currentAccount.sum = currentAccountTransactions.reduce(
      (sum, a) => (a.type === 'expense' ? sum - a.sum : sum + a.sum),
      0
    );
    ...
    for (let i = 0; i < accounts.length; i++) {
      let transactions = db
        .get('transactions')
        .filter({ account_id: accounts[i].id })
        .value();
      accounts[i].sum = transactions.reduce(
        (sum, a) => (a.type === 'expense' ? sum - a.sum : sum + a.sum),
        0
      );
    }
