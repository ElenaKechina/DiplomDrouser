...
router.post('/register', upload.none(), function (request, response) {
  ...
  if (!user) {
    //если существующий пользователь не найден...
    ...
    //если существующий пользователь найден...
    //Отправляется ошибка о том, что пользователь такой уже существует
    response.json({
      success: false,
      error: `E-Mail адрес ${email} уже существует.`,
    });
  }
});
...
router.get('/current', function (request, response) {
  ...
  if (userValue) {
    //если пользователь найден и он авторизован...
